...
test('the 1024 Store API is preferred over public registry fallbacks', () => {
  ...
  // The last-resort leg reads the CURRENT repo layout (plugin/); the frozen
  // packages/dsh1024 segment lives only in identity strings like releaseUrl.
  assert.match(DEFAULT_UPDATE_LAST_RESORT_URL, /repos\/imsai-sh\/dsh-1024store\/contents\/plugin\/package\.json/)
  assert.equal(
    DEFAULT_RELEASE_URL,
