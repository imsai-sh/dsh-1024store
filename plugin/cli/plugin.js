...
export async function forwardPluginCommand(command, context) {
  ...
  try {
    ...
    if (effectiveTelemetryEnabled(telemetryConfig, env) && await markNoticeShown(dshHome, telemetryConfig, now)) {
      stderr('DSH 1024Store records anonymous plugin install outcomes and timestamps. Disable with `npx dsh1024 telemetry disable`, `DO_NOT_TRACK=1`, or `DSH1024_TELEMETRY=0`. Details: https://github.com/imsai-sh/dsh-1024store/blob/main/web/docs/install-analytics.md')
    }
  } catch {
