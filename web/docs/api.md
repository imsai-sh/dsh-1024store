...
`web/contracts/` and are enforced by `npm run test:api-contract`.
...
id is neither in the D1 `catalog_categories` table nor `unclassified` (for example, a
category that was removed from the definitions after plugins referenced it).
...
- The event schema (19 fields) is unchanged; see
  ...
  `web/worker/lib/install-metrics.ts` for the authoritative validation code.
...
- `200 {"ok": true, "total": N, "removedSources": M}` on success — plus `"categories": K`
  when the request carried a `categories` field.

Request body (produced by the catalog repository's `scripts/sync-catalog.mjs` in
imsai-sh/awesome-deepseek-harness-plugins from its `catalog/categories.json` and
`catalog/plugins/*.json`, with the `$schema` field stripped):
...
`categories` is optional. When present it is the FULL ordered category list: entries are
validated against it, and once the whole request validates, the D1 `catalog_categories`
table is reconciled to exactly it — so a sync may introduce a category and use it in the
same request, and a rejected request never half-writes the table. Constraints: 1 to 200
items; ids match `[a-z][a-z0-9-]*` (at most 40 characters) and must be unique;
`unclassified` is reserved for the Worker's synthetic bucket; `order` is an integer in
`[0, 1000000]`; each label carries non-empty `en` and `zh` strings of at most 120
characters. When the field is absent, entries validate against the current table contents
(a Worker whose D1 lacks the table answers `503 {"error": "Category definitions are
unavailable."}`). Workers deployed before this field existed reject the whole request with
`400 Unexpected field: categories.` — deploy the Worker before the catalog repository
starts sending it.
