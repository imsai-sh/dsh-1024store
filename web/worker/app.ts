...
import {
  categoryDescriptorFrom,
  isKnownCategoryIdIn,
  loadCategoriesFromD1,
  projectCategories,
...
interface AppDependencies {
  ...
  categoryReconciler: typeof reconcileCategories
  selfUpdateLoader: (
    db: D1Database,
...
const SELF_CATALOG_PLUGIN_ID_LIST = [
  // Frozen identity: `packages/dsh1024` is the HISTORICAL in-repo path baked
  // into live D1 rows and every shipped client. It survives directory renames
  // (the package now lives at plugin/); never "modernize" this string.
...
function parseCuratedEntry(
  value: unknown,
  index: number,
  knownCategoryIds: ReadonlySet<string>,
): CuratedCatalogEntry | string {
  ...
  if (!boundedString(value.category, 40) || !knownCategoryIds.has(value.category)) {
...
// Mirrors catalog/categories.json in the catalog repository; the sync workflow
// sends the full ordered list and the table is reconciled to exactly it.
const CATEGORY_ID = /^[a-z][a-z0-9-]{0,39}$/
const MAX_SYNC_CATEGORIES = 200

type SyncCategoriesParseResult =
  | { ok: true; categories: CategoryDescriptor[] | null }
  | { ok: false; error: string }

function parseSyncCategories(value: Record<string, unknown>): SyncCategoriesParseResult {
  if (!('categories' in value)) return { ok: true, categories: null }
  const raw = value.categories
  if (!Array.isArray(raw) || raw.length === 0 || raw.length > MAX_SYNC_CATEGORIES) {
    return { ok: false, error: 'categories must be a non-empty array.' }
  }
  const categories: CategoryDescriptor[] = []
  const seen = new Set<string>()
  for (const [index, item] of raw.entries()) {
    if (!isObject(item)) return { ok: false, error: `Category ${index} must be a JSON object.` }
    const unexpected = Object.keys(item).find((key) => key !== 'id' && key !== 'order' && key !== 'label')
    if (unexpected) return { ok: false, error: `Category ${index} has an unexpected field: ${unexpected}.` }
    if (typeof item.id !== 'string' || !CATEGORY_ID.test(item.id) || item.id === UNCLASSIFIED_CATEGORY.id) {
      return { ok: false, error: `Category ${index} has an invalid id.` }
    }
    if (seen.has(item.id)) return { ok: false, error: `Category ${index} duplicates id ${item.id}.` }
    if (!Number.isInteger(item.order) || (item.order as number) < 0 || (item.order as number) > 1_000_000) {
      return { ok: false, error: `Category ${index} has an invalid order.` }
    }
    const label = item.label
    if (!isObject(label) || !boundedString(label.en, 120) || !boundedString(label.zh, 120)) {
      return { ok: false, error: `Category ${index} has an invalid label.` }
    }
    seen.add(item.id)
    categories.push({ id: item.id, order: item.order as number, label: { en: label.en, zh: label.zh } })
  }
...
function parseCatalogSyncRequest(
  ...
  const unexpected = Object.keys(value)
    .find((key) => key !== 'source' && key !== 'entries' && key !== 'categories')
  ...
  for (const [index, item] of value.entries.entries()) {
    const parsed = parseCuratedEntry(item, index, knownCategoryIds)
...
export function createApp(overrides: Partial<AppDependencies> = {}) {
  const dependencies: AppDependencies = {
    ...
    categoryReconciler: reconcileCategories,
  ...
  app.get('/api/v1/plugins/search', async (context) => {
    ...
    // Category validity comes from the snapshot (D1-backed since migration
    // 0014). Checked AFTER the quota so an invalid-category probe still draws
    // down the caller's window like any other 4xx, and reusing the load above
    // keeps the check I/O-free.
    if (category && !isKnownCategoryIdIn(snapshotResult.snapshot.categoryList, category)) {
  ...
  app.get('/api/v1/plugins/:owner/*', async (context) => {
    ...
    return context.json({
      ...detail,
      ...installMetrics,
      category: categoryDescriptorFrom(snapshot.snapshot.categoryList, plugin.category),
    })
  })
  ...
  app.get('/api/v1/registry', async (context) => {
    ...
    const registry: RegistryProjection = {
      ...
      categories: projectCategories(snapshot.categories, snapshot.categoryList),
  ...
  app.post('/api/v1/catalog/sync', async (context) => {
    ...
    if (!isObject(value)) return context.json({ error: 'Request body must be a JSON object.' }, 400)
    // Entries are validated against the category list carried by the request
    // (falling back to the current table when absent); the table is reconciled
    // only once the whole request validates. A sync may therefore introduce a
    // category and use it in the same request, and a rejected request never
    // half-writes the category table.
    const parsedCategories = parseSyncCategories(value)
    if (!parsedCategories.ok) return context.json({ error: parsedCategories.error }, 400)
    let knownCategoryIds: Set<string>
    if (parsedCategories.categories !== null) {
      knownCategoryIds = new Set(parsedCategories.categories.map((category) => category.id))
    } else {
      try {
        knownCategoryIds = new Set(
          (await loadCategoriesFromD1(context.env.CATALOG_DB)).map((category) => category.id),
        )
      } catch {
        // Schema problem on OUR side (0014 not applied / D1 unavailable) — a
        // 400 here would blame the catalog repo's data and stall its CI on a
        // fault it cannot fix.
        return context.json({ error: 'Category definitions are unavailable.' }, 503)
      }
    }
    const parsed = parseCatalogSyncRequest(value, knownCategoryIds)
    if (!parsed.ok) return context.json({ error: parsed.error }, 400)

    const capturedAt = dependencies.clock()
    if (parsedCategories.categories !== null) {
      await dependencies.categoryReconciler(
        context.env.CATALOG_DB,
    ...
    return context.json({
      ...
      ...(parsedCategories.categories === null ? {} : { categories: parsedCategories.categories.length }),
    })
  })
