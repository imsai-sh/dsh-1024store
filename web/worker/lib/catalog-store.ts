...
// v11 adds `categoryList`: category definitions now ride the snapshot from D1
// (migration 0014) instead of being bundled into the Worker. Starting with a
// fresh key prevents an old v10 snapshot — which has no categoryList — from
// serving a Worker that requires it. (v10 dropped npm download metrics from
// repositories that do not own the published package, for the same reason.)
const SNAPSHOT_KEY = 'catalog:snapshot:v11'
...
function isCategoryListEntry(value: unknown): boolean {
  return (
    isObject(value) &&
    typeof value.id === 'string' &&
    typeof value.order === 'number' &&
    isLocalizedCategory(value.label)
  )
}
...
export function isStoredCatalogSnapshot(value: unknown): value is StoredCatalogSnapshot {
  ...
  return (
    ...
    Array.isArray(value.categoryList) &&
    value.categoryList.length > 0 &&
    value.categoryList.every(isCategoryListEntry) &&
