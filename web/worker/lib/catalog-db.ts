...
import { categoryLabelMap, loadCategoriesFromD1, UNCLASSIFIED_CATEGORY } from './categories'
...
export async function loadCatalogSnapshotFromD1(
  ...
  const categoryList = await loadCategoriesFromD1(db)
  // A catalog without categories is a schema problem (0014 not applied, or the
  // table truncated out of band), never a valid state: refusing here makes the
  // rebuild fail loudly and degrade to the previous snapshot instead of
  // persisting a poisoned one that reads would never rebuild.
  if (categoryList.length === 0) {
    throw new Error('catalog_categories is empty; refusing to build a snapshot without categories')
  }
  const categories = categoryLabelMap(categoryList)
...
/**
 * Full reconciliation of the category definitions into D1: the table ends up
 * holding exactly `categories`, in a single batch so readers never observe a
 * half-applied list. Fed by the catalog repository's sync workflow; idempotent.
 */
export async function reconcileCategories(
  ...
  // The endpoint rejects an empty list too, but the invariant lives with the
  // write: an empty catalog_categories table is the poisoned-snapshot state.
  if (categories.length === 0) {
    throw new Error('refusing to reconcile catalog_categories to an empty list')
  }
  const keptIds = JSON.stringify(categories.map((category) => category.id))
  await db.batch([
    db.prepare('DELETE FROM catalog_categories WHERE id NOT IN (SELECT value FROM json_each(?))').bind(keptIds),
    ...categories.map((category) => db.prepare(
      `INSERT INTO catalog_categories (id, sort_order, label_en, label_zh, updated_at)
       VALUES (?, ?, ?, ?, ?)
       ON CONFLICT(id) DO UPDATE SET
         ...
         updated_at = excluded.updated_at`,
    ).bind(category.id, category.order, category.label.en, category.label.zh, now)),
  ])
  // A removed or renamed id can orphan rows the collection pipeline classified
  // (ai_category is never validated by the sync). Orphans still project — the
  // snapshot falls back to order-1000 descriptors — but the condition should
  // be visible, not silent.
  const orphaned = await db.prepare(
    `SELECT COUNT(*) AS count FROM catalog_plugins
      WHERE COALESCE(curated_category, ai_category) IS NOT NULL
        AND COALESCE(curated_category, ai_category) NOT IN (SELECT id FROM catalog_categories)`,
  ).first<{ count: number }>()
  if (orphaned !== null && orphaned.count > 0) {
    console.error(JSON.stringify({ message: 'orphaned_plugin_categories', count: orphaned.count }))
  }
