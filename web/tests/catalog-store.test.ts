    ...
    const statement = {
      ...
      async all() {
        ...
        if (sql.includes('FROM catalog_categories')) {
          return {
            results: [
              { id: 'ui', sort_order: 10, label_en: 'UI Enhancements', label_zh: 'UI 增强' },
              { id: 'model', sort_order: 90, label_en: 'Models & Providers', label_zh: '模型与账号接入' },
            ],
          }
        }
...
describe('catalog snapshot storage', () => {
  ...
  it('builds the snapshot from D1 with categories sourced from the catalog_categories table', async () => {
  ...
  it('refuses to persist a snapshot when catalog_categories is missing or empty', async () => {
    // The pre-0014 window: D1 answers the plugin query but has no category
    // table. The rebuild must fail (degrading to the stale snapshot) instead
    // of persisting a poisoned zero-category snapshot that reads never rebuild.
    const stale = {
      ...testCatalogResult().snapshot,
      generatedAt: '2026-08-14T00:00:00.000Z',
    }
    const put = vi.fn(async () => undefined)
    const brokenDb = snapshotDb([catalogRow()])
    ;(brokenDb.prepare as ReturnType<typeof vi.fn>).mockImplementation((sql: string) => ({
      bind() { return this },
      async all() {
        if (sql.includes('FROM catalog_categories')) throw new Error('no such table: catalog_categories')
        if (sql.includes('FROM catalog_plugins')) return { results: [catalogRow()] }
        return { results: [] }
      },
  ...
  it('serves an explicitly empty snapshot when both D1 and KV are unavailable', async () => {
    ...
    // Categories are D1 data now: with D1 unavailable the empty snapshot
    // carries none instead of a bundled fallback.
    expect(result.snapshot.categories).toEqual({})
    expect(result.snapshot.categoryList).toEqual([])
    expect(put).not.toHaveBeenCalled()
  })
