...
import { TEST_CATEGORY_LIST, TEST_PLUGINS, testCatalogResult } from './fixtures'
...
// The sync handler reads catalog_categories from D1 when the request body
// carries no categories field; the mock serves the fixture list (D1-backed
  ...
  CATALOG_DB: {
    prepare: (sql: string) => {
      const statement = {
        bind: () => statement,
        all: async () => ({
          results: sql.includes('FROM catalog_categories')
            ? TEST_CATEGORY_LIST.map((category) => ({
                id: category.id,
                sort_order: category.order,
                label_en: category.label.en,
                label_zh: category.label.zh,
              }))
            : [],
        }),
...
function syncApp() {
  ...
  const categoryReconciler = vi.fn(async (
    _db: D1Database,
  ...
  return { app, curatedSyncer, categoryReconciler, snapshotRefresher }
}
...
describe('market API', () => {
  ...
  it('projects the compact registry with categories and install commands', async () => {
    ...
    // The FULL id set is pinned, not just the head: categories are D1 data
    // now, and a list silently shrinking must fail here. (No unclassified
    // entry: every fixture plugin is categorized, and the synthetic bucket
    // only appears in the map when an uncategorized plugin exists.)
    expect(body.categories.map((category) => category.id))
      .toEqual(TEST_CATEGORY_LIST.map((category) => category.id))
  ...
  it('reconciles a pushed category list before validating entries against it', async () => {
    const { app, curatedSyncer, categoryReconciler } = syncApp()
    const categories = [
      { id: 'ui', order: 10, label: { en: 'UI Enhancements', zh: 'UI 增强' } },
      { id: 'brand-new', order: 15, label: { en: 'Brand New', zh: '全新分类' } },
    ]
    // The entry uses the category introduced by the SAME request: reconciliation
    // must come first, or a new category could never be used by its own sync.
    const response = await app.request(
      '/api/v1/catalog/sync',
      syncRequest({
        source: 'github_ci',
        categories,
        entries: [{ ...VALID_SYNC_ENTRY, category: 'brand-new' }],
      }),
      SYNC_ENV,
    )
    expect(response.status).toBe(200)
    ...
    expect(categoryReconciler).toHaveBeenCalledOnce()
    expect(categoryReconciler.mock.calls[0]?.[1]).toEqual(categories)
    expect(curatedSyncer).toHaveBeenCalledOnce()
  })

  it('answers 503 when the category table is unavailable and the request carries none', async () => {
    const { app, curatedSyncer } = syncApp()
    const brokenDbEnv = {
      CATALOG_DB: {
        prepare: () => ({
          bind() { return this },
          all: async () => { throw new Error('no such table: catalog_categories') },
          run: async () => { throw new Error('no such table: catalog_categories') },
        }),
        batch: async () => { throw new Error('no such table: catalog_categories') },
      },
      CATALOG_SYNC_TOKEN: SYNC_TOKEN,
    } as unknown as Env
    const response = await app.request(
    ...
    expect(response.status).toBe(503)
    await expect(response.json()).resolves.toEqual({ error: 'Category definitions are unavailable.' })
    expect(curatedSyncer).not.toHaveBeenCalled()
  })

  it('rejects malformed category lists and the synthetic unclassified id', async () => {
    const { app, curatedSyncer, categoryReconciler } = syncApp()
    for (const categories of [
      [],
      [{ id: 'unclassified', order: 1000, label: { en: 'Unclassified', zh: '待分类' } }],
      [{ id: 'UI', order: 10, label: { en: 'x', zh: 'y' } }],
      [{ id: 'ui', order: 1.5, label: { en: 'x', zh: 'y' } }],
      [{ id: 'ui', order: 10, label: { en: '', zh: 'y' } }],
      [
        { id: 'ui', order: 10, label: { en: 'x', zh: 'y' } },
        { id: 'ui', order: 20, label: { en: 'x2', zh: 'y2' } },
      ],
    ]) {
      const response = await app.request(
        '/api/v1/catalog/sync',
        syncRequest({ source: 'github_ci', categories, entries: [VALID_SYNC_ENTRY] }),
        SYNC_ENV,
      )
      expect(response.status).toBe(400)
    }
    expect(categoryReconciler).not.toHaveBeenCalled()
    expect(curatedSyncer).not.toHaveBeenCalled()
  })
