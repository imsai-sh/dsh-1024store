...
import { loadCategoriesFromD1 } from '../worker/lib/categories'
...
function catalogDatabase(): DatabaseSync {
  ...
  for (const migration of ['0002_plugin_catalog.sql', '0005_catalog_plugins.sql',
    ...
    '0013_relax_plugin_id_uniqueness.sql', '0014_catalog_categories.sql']) {
...
describe('catalog snapshot', () => {
  it('loads a published package version directly without rebuilding the snapshot', async () => {
    ...
    seedPlugin(database, 'imsai-sh/awesome-deepseek-harness-plugins', {
      // Frozen identity path: production D1 keys the self plugin under the
      // historical packages/dsh1024 segment regardless of the repo layout.
...
describe('category definitions in D1', () => {
  it('seeds migration 0014 and reconciles the table to exactly the pushed list', async () => {
    const database = catalogDatabase()
    const db = sqliteD1(database)

    const seeded = await loadCategoriesFromD1(db)
    expect(seeded.map((category) => category.id)).toContain('ui')
    expect(seeded).toHaveLength(11)
    expect(seeded[0]).toEqual({ id: 'ui', order: 10, label: { en: 'UI Enhancements', zh: 'UI 增强' } })

    await reconcileCategories(db, [
      { id: 'fresh', order: 1, label: { en: 'Fresh', zh: '新分类' } },
      { id: 'ui', order: 5, label: { en: 'Renamed UI', zh: '改名 UI' } },
    ], '2026-08-25T00:00:00.000Z')

    const after = await loadCategoriesFromD1(db)
  ...
  it('propagates a missing table instead of minting an empty list', async () => {
    const database = new DatabaseSync(':memory:')
    const db = sqliteD1(database)
    await expect(loadCategoriesFromD1(db)).rejects.toThrow()
    database.close()
  })

  it('refuses to reconcile to an empty list', async () => {
    const database = catalogDatabase()
    const db = sqliteD1(database)
    await expect(reconcileCategories(db, [], '2026-08-25T00:00:00.000Z')).rejects.toThrow(/empty list/)
    expect(await loadCategoriesFromD1(db)).toHaveLength(11)
    database.close()
  })
})
